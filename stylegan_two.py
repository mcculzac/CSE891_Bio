...
def upsample_to_size(x):
    y = im_size / x.shape[2]
    x = K.resize_images(x, int(y), int(y), "channels_last",interpolation='bilinear')
    return x
...
class GAN(object):
    ...
    def generator(self):
        ...
        self.S.add(Dense(128, input_shape=[latent_size]))
        self.S.add(LeakyReLU(0.2))
        # self.S.add(Dense(512))
        self.S.add(Dense(128))
        self.S.add(LeakyReLU(0.2))
        # self.S.add(Dense(512))
        self.S.add(Dense(128))
        self.S.add(LeakyReLU(0.2))
